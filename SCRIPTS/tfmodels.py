...
def np_generate_latent_points(latent_dim, n_samples):
...
def np_generate_real_samples(data, n) :
    return data[np.random.choice(data.shape[0], n, replace=False), :], -np.ones((n,1))

def generate_latent_points(latent_dim, n_samples):
    return tf.random.normal([n_samples, latent_dim])

def generate_real_samples(data, n) :
    return tf.random.shuffle(data)[:n,:], -tf.ones((n,1))

def generate_fake_samples(g_model, latent, n) :
    return g_model(generate_latent_points(latent, n)), tf.ones((n,1))
...
def wgan(input_data, my_activation="relu", ae_act="linear", latent_dim_size=10000):
    ...
    generator_input = tf.keras.layers.Input(shape=(latent_dim_size,))
